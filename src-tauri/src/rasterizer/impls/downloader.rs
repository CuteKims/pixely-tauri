use std::{fs::{create_dir_all, File}, io::Write, sync::atomic::{AtomicUsize, Ordering}, thread::{self, sleep}, time::Duration};

use futures::{stream, StreamExt};

use crate::rasterizer::models::{downloader::{ConcurrentDownloader, DownloadManager}, parser::resource_parser::DownloadIndex};
...
impl ConcurrentDownloader {
    async fn download(self, counter: &AtomicUsize) -> Result<Vec<DownloadIndex>, Box<dyn std::error::Error>> {
...
impl DownloadManager {
    async fn download(self) -> Result<(), Box<dyn std::error::Error>> {
        let total_bytes = self.download_index.clone().into_iter().map(|index| {index.size.unwrap()}).sum::<usize>();
